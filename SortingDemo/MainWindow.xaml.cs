        ...
        private async void StartButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (selectedAlgorithm == "SelectSort")
                {
                    ...
                    log = await SortingAlgorithms.SelectSort(array, VisualizeStep, delay, SortingLog);
                }
                else if (selectedAlgorithm == "BubbleSort")
                {
                    log = await SortingAlgorithms.BubbleSort(array, VisualizeStep, delay, SortingLog);
                }
                else if (selectedAlgorithm == "QuickSort")
                {
                    log = await SortingAlgorithms.QuickSort(array, 0, array.Length - 1, VisualizeStep, delay, SortingLog);
                }
                else if (selectedAlgorithm == "HeapSort")
                {
                    log = await SortingAlgorithms.HeapSort(array, VisualizeStep, delay, SortingLog);
                }
